...
describe('Action Creators', () => {
  ...
  describe('addColor', () => {
    const colors = [
      {
        ...
        color: '#44ef37',
        timestamp:
          'Mon Apr 11 2016 12:54:19 GMT-0700 (PDT)',
        rating: 4,
      },
      {
        ...
        color: '#0061ff',
        timestamp:
          'Mon Apr 11 2016 12:54:31 GMT-0700 (PDT)',
        rating: 2,
      },
      {
        ...
        color: '#ff4b47',
        timestamp:
          'Mon Apr 11 2016 12:54:43 GMT-0700 (PDT)',
        rating: 0,
      },
    ];
    ...
    afterAll(
      () => (localStorage['redux-store'] = false),
    );

    it('should add a new color', () => expect(store.getState().colors.length).toBe(4));

    it('should add a unique guid id', () => expect(store.getState().colors[3].id.length).toBe(
      36,
    ));

    it('should set the rating to 0', () => expect(store.getState().colors[3].rating).toBe(0));

    it('should set timestamp', () => expect(
      store.getState().colors[3].timestamp,
    ).toBeDefined());
  });
  ...
  describe('removeColor', () => {
    ...
    beforeAll(() => {
      store = storeFactory({ colors });
      store.dispatch(
        removeColor('8658c1d0-9eda-4a90-95e1-8001e8eb6036'),
      );
    });

    it('should remove the color', () => expect(store.getState().colors.length).toEqual(0));
  });
